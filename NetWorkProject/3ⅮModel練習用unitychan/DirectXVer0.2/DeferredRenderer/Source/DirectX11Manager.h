...
public:
	...
	void UpdateConstantBuffer(ID3D11Buffer* buffer,
		const x& cb)
	{
		if (m_pImContext == nullptr || buffer==nullptr) {
			//何もしない、コンパイル中
		}
		else {
	...
	//bufferCreate
	template<class x>
	ID3D11Buffer* CreateVertexBuffer(x* VertexData,
		UINT VertexNum)
	{
		//頂点バッファ作成
	...
	ID3D11Buffer* CreateIndexBuffer(UINT* Index,
		UINT IndexNum)
	{
		//インデックスバッファ作成
		D3D11_BUFFER_DESC hBufferDesc;
	...
	ID3D11Texture2D* CreateTexture2D(UINT Width,
