	...
	_In_ LPWSTR lpCmdLine, _In_ int nShowCmd)
{
	...
	//全てのプレイヤーデータ
	SendData* BULLET_ALL = new SendData();
	...
	while (CheckHitKey(KEY_INPUT_ESCAPE) == 0) {
		...
		if (NetHandel != -1) {
			...
			memcpy_s(BULLET_ALL, sizeof(SendData), StrBuf, sizeof(SendData));//変換
	...
	while (CheckHitKey(KEY_INPUT_ESCAPE) == 0) {
		...
		if (GetNetWorkDataLength(NetHandel) != 0) {
			...
			//プレイヤー全体データの更新
			memcpy_s(BULLET_ALL, sizeof(SendData), StrBuf, sizeof(SendData));
			net_Receive = true;
		}
		...
		//Player_ALLを使って画面の更新
		for (int i = 0; i < MAX; i++) {
			if (BULLET_ALL->data[i].ID != -1) {
				//キャラ
				DrawGraphF(BULLET_ALL->data[i].pos.x,
					BULLET_ALL->data[i].pos.y,
					img[i],
					TRUE
				);
				//名前
				DrawStringF(BULLET_ALL->data[i].pos.x,
					BULLET_ALL->data[i].pos.y,
					BULLET_ALL->data[i].name,
					GetColor(255, 255, 255)
				);
			}
		...
		DrawGraphF(0.0f, 0.0f, img[BULLET_ALL->data->ID], TRUE);
